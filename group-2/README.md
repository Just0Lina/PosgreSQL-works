...
There is a SQL statement that returns the total volume (sum of all money) of transactions from user balance aggregated by user and balance type. All data processed including data with anomalies. Below presented a table of result columns and corresponding calculation formula.
...
Result sorted by user name in descending mode and then by user lastname and balance type in ascending mode.
...
There is a SQL statement that returns all Users , all Balance transactions (in this task please ignore currencies that do not have a key in the `Currency` table ) with currency name and calculated value of currency in USD for the nearest day.
...
Result sorted by user name in descending mode and then by user lastname and currency name in ascending mode.
